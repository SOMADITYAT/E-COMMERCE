          ...
          <div className="rounded-lg md:w-2/3 ">
           {
            cartItems.map((item,index) => {
              const {title,price, descripation , imageUrl}= item;
              ...
              return(
                ...
                <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                  <div className="mt-5 sm:mt-0">
                    ...
                    <h2 className="text-sm  text-gray-900" style={{ color: mode === 'dark' ? 'white' : '' }}>{descripation}</h2>
                    <p className="mt-1 text-xs font-semibold text-gray-700" style={{ color: mode === 'dark' ? 'white' : '' }}>{price}</p>
                  </div>
                  <div onClick={()=> deletecCart(item)} className="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
